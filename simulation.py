...
class SensorDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tensor = [torch.FloatTensor(self.image_transform(
            transforms.ToPILImage()(t))) for t in tensor]
    ...
    def __init__(self, model_name, transfer, num_input_channel, num_output_class, dropout):
        ...
        if bool(transfer) == True:
            if model_name == 'Xception':
                self.model = pretrainedmodels.__dict__['xception'](
                    num_classes=1000, weights='imagenet')
            ...
            if model_name == 'Xception':
                self.model = pretrainedmodels.__dict__[
                    'xception'](num_classes=1000, weights=None)
            elif model_name == 'Resnet18':
                ...
                self.model = efficientnet_pytorch.EfficientNet.from_name(
                    'efficientnet-b0')
            elif model_name == 'Efficientnet-b4':
                self.model = efficientnet_pytorch.EfficientNet.from_name(
                    'efficientnet-b4')
        ...
        if model_name in ['Xception']:
            ...
            self.model.last_linear = custom_cf(
                self.model.last_linear.in_features, num_output_class, dropout)
        elif model_name in ['Resnet18', 'Resnet34', 'Resnet50', 'Resnet101']:
            input_layer = nn.Conv2d(num_input_channel, self.model.conv1.out_channels, kernel_size=3, stride=1,
                                    padding=1, dilation=1, groups=1, bias=True)
            self.model.conv1 = input_layer
            self.model.fc = custom_cf(
                self.model.fc.in_features, num_output_class, dropout)
            ...
            self.model._fc = custom_cf(
                self.model._fc.in_features, num_output_class, dropout)
...
def model_run(model_name, transfer, noise):
    ...
    print('\nModel: {} | Transfer Learning: {} | Noisy Data: {:.2f}'.format(
        model_name, transfer, noise))
    ...
    if not os.path.exists(os.path.join(os.getcwd(), 'snapshot', '{}_{}'.format(model_name, transfer))):
        os.mkdir(os.path.join(os.getcwd(), 'snapshot',
                 '{}_{}'.format(model_name, transfer)))
    ...
    for i in range(5):
        ...
        class_weights = torch.FloatTensor(pd.value_counts(
            train_tensor_array[:, 1], normalize=True).sort_index())
        ...
        train_sensor_dataset = SensorDataset(
            train_tensor_array, image_transform)
        valid_sensor_dataset = SensorDataset(
            valid_tensor_array, image_transform)
        test_sensor_dataset = SensorDataset(test_tensor_array, image_transform)

        BATCH_SIZE = 4
        train_batch = DataLoader(
            train_sensor_dataset, batch_size=BATCH_SIZE, shuffle=True, drop_last=True)
        valid_batch = DataLoader(
            valid_sensor_dataset, batch_size=BATCH_SIZE, shuffle=True, drop_last=False)
        test_batch = DataLoader(
            test_sensor_dataset, batch_size=BATCH_SIZE, shuffle=True, drop_last=False)
        ...
        model = TransferModel(model_name, transfer,
                              num_input_channel, num_output_class, dropout)
        ...
        for e in range(0, EPOCHS):
            ...
            if not best_valid_loss or valid_loss < best_valid_loss:
                ...
                torch.save(model.state_dict(), os.path.join(
                    os.getcwd(), 'snapshot', '{}_{}'.format(model_name, transfer), out_name))
                best_valid_loss = valid_loss

            # Decay Learning Rate
            scheduler.step()

        trained_weight = os.path.join(
            os.getcwd(), 'snapshot', '{}_{}'.format(model_name, transfer), out_name)
        state_dict = torch.load(trained_weight)
        model.load_state_dict(state_dict)
        test_loss, test_accuracy, true_list, pred_list = evaluate(
            model, criterion, test_batch)
        print("\n test loss : %5.5f | test accuracy : %5.5f" %
              (test_loss, test_accuracy))

        test_result_path = os.path.join(os.getcwd(), 'results', '{}_{}'.format(
            model_name, transfer), str('test_result_{:0.2f}_{}.pkl'.format(noise, i)))
...
if __name__ == '__main__':
    model_name_set = ['Resnet18', 'Resnet50']
